(() => {
  ...
  function renderCalendar(){
    ...
    for (let day=1; day<=last.getDate(); day++){
      ...
      const colorMap = {
        PR: '#f2d34b',  // Πρωί (κίτρινο)
        AP: '#f39b4a',  // Απόγευμα (πορτοκαλί)
        NY: '#0f8b8d',  // Νύχτα (πρασινο-μπλε)
        AD: '#79c7ff',  // Αδιάθετο (γαλάζιο)
        RP: '#e13b3b',  // Ρεπό (κόκκινο)
      };
      ...
      if(arr && arr.length){
        ...
        if(colors.length===1){
          div.style.setProperty('--shift-bg', colors[0]);
          div.style.setProperty('--shift-fg', isLightColor(colors[0]) ? '#0b1f2a' : '#ffffff');
        } else if(colors.length>=2){
          div.style.setProperty('--shift-bg', `linear-gradient(135deg, ${colors[0]} 0 50%, ${colors[1]} 50% 100%)`);
        }
  ...
  function fillCycle(){
    ...
    const length = Number($('cycleLength')?.value || 5); // 5 ή 10
    const start = parseYMD(startStr);

    // read pattern (5 ή 10 ημέρες) από ό,τι έχεις ήδη συμπληρώσει
    const pattern = [];
    ...
    // πρέπει να υπάρχει τουλάχιστον 1 ημέρα με βάρδια για να έχει νόημα
    if (!pattern.some(arr => arr.length)){
      return alert(`Δεν υπάρχουν βάρδιες στις πρώτες ${length} ημέρες για να γίνει κύκλος.`);
    }

    // Τέλος γέμισματος: start + months
    const end = new Date(start);
    end.setMonth(end.getMonth()+months);

    // fill from start onward by repeating pattern (χωρίς να πειράζει υπάρχουσες ημέρες)
    let cursor = new Date(start);
    cursor.setDate(cursor.getDate()+length); // δεν πειράζουμε το αρχικό pattern
    ...
    while (cursor < end){
      ...
      cursor.setDate(cursor.getDate()+length);
    }
