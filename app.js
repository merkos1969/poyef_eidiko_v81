...
(function(){
  ...
  function calcCounts(y,m,data){
    ...
    for(const k of Object.keys(data||{})){
      ...
      const dow = d.getDay(); // 0 Sun,5 Fri,6 Sat
      const shifts = getShifts(data,k);
      const hasAnyService = shifts.length>0 && !shifts.includes("RP");

      // 5Μ rules
      // Fri NY only => 5Μ (no night pay)
      if(dow===5 && shifts.length===1 && shifts[0]==="NY"){
        fives += 1;
      }

      // Weekend service => 5Μ (Sat/Sun with any service)
      if((dow===6 || dow===0) && hasAnyService){
        fives += 1;
      }

      // Night pay rules:
      // - Fri NY: no night pay (already handled above)
      // - Sat NY alone: no night pay (still gets 5Μ from weekend rule)
      // - Sat PR+NY: counts 1 night pay (special rule)
      if(shifts.includes("NY")){
        if(dow===5){
          // Friday NY -> no night pay
        }else if(dow===6){
          if(shifts.includes("PR")) nights += 1; // Saturday PR+NY => 1 night
        }else{
          // Sun–Thu night counts
          nights += 1;
        }
      }
    }
  ...
  function renderCal(){
    ...
    Array.from(calGrid.querySelectorAll(".calCell[data-key]")).forEach(cell=>{
      cell.addEventListener("click", ()=>{
        ...
        const curArr = getShifts(data,k);
        if(activeShift==="PR" || activeShift==="NY"){
          const set = new Set(curArr);
          if(set.has(activeShift)) set.delete(activeShift);
          else{
            // keep only PR/NY for combo, remove other types
            ["AP","AD","RP"].forEach(x=>set.delete(x));
            set.add(activeShift);
          }
          setShifts(data,k,Array.from(set));
        }else{
          // other shifts are single-toggle
          if(curArr.length===1 && curArr[0]===activeShift) setShifts(data,k,[]);
          else setShifts(data,k,[activeShift]);
        }
        saveMonth(y,m,data);
        renderCal();
      });
    ...
    // Auto transfer to payroll
    const inpNight = document.getElementById("nightHours");
    const inpFive  = document.getElementById("fiveDays");
    if(inpNight) inpNight.value = String(nights);
    if(inpFive)  inpFive.value  = String(fives);
  ...
  if(applyBtn){
    // kept for compatibility, but auto-transfer already happens
