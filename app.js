  ...
  function renderCal(){
    ...
    for(let r=0;r<6;r++){
      ...
      for(let c=0;c<7;c++){
        ...
        const badgeItems = arrShifts.slice(0,2).map(t=>`<div class="badge ${t}">${labelOf(t)}</div>`).join("");
        const badgesHtml = badgeItems ? `<div class="badgesRow">${badgeItems}</div>` : "";
    ...
    Array.from(calGrid.querySelectorAll(".calCell[data-key]")).forEach(cell=>{
      cell.addEventListener("click", ()=>{
        ...
        // add (no delete on second tap)
        const idx = arr.indexOf(activeShift);
        if(idx>=0){
          // ήδη υπάρχει -> δεν την αφαιρούμε με δεύτερο πάτημα (αφαίρεση γίνεται από Καθαρισμό)
          return;
        } else {
