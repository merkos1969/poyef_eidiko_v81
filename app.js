...
function calc() {
  ...
  const payslipMonthhh = Number(document.getElementById('payslipMonthh')?.value || 0); // 0=Ιαν, 11=Δεκ
  const payslipYear = Number(document.getElementById('payslipYear')?.value || new Date().getFullYear());
  const payslipDate = new Date(payslipYear, payslipMonthh, 1);
...
// Σταθερές κρατήσεις
  ...
  const taxableMonthlyBase = Math.max(0, grossTotal - (dedMain + dUnempExtra + dMtpyExtra));
  const annualIncome = taxableMonthlyBase * 12; // φορολογητέο: (μεικτά - κρατήσεις) * 12
...
function printPayslip(){
  ...
  const monthName = document.getElementById('payslipMonthh').value;
...
function init(){
  ...
  document.addEventListener('input', (e)=>{
    const ids = new Set(['appointDate','status','children','after1993','border','fiveDays','nightHours','loanMtpy','fiveRecognize','payslipMonthh','payslipYear','applyTax']);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // επανυπολογισμός σε κάθε αλλαγή
  const ids = [
    'appointDate','years','grade','step','status','children','after1993','border','fiveDays','nightHours',
    'recognition','mtpyLoan','applyTax','taxAge','taxKids','payslipMonth','payslipYear','neoMtpyEnabled'
  ];
  ids.forEach(id => {
    const el = document.getElementById(id);
    if(el) el.addEventListener('input', () => { updateBasicAnalysis(); calc(); });
    if(el) el.addEventListener('change', () => { updateBasicAnalysis(); calc(); });
  });
});
