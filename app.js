...
function calc() {
  ...
  // Νεοδιόριστος ΜΤΠΥ: 1/12 του βασικού για τους πρώτους 12 μήνες από την ημερομηνία διορισμού
  const appointDate = document.getElementById('appointDate').value;
  const appointDt = appointDate ? new Date(appointDate) : null;
  const nowDt = new Date();
  const monthsFromAppoint = appointDt ? ((nowDt.getFullYear()-appointDt.getFullYear())*12 + (nowDt.getMonth()-appointDt.getMonth()) + (nowDt.getDate()>=appointDt.getDate()?0:-1)) : 999;
  const dMtpyNeo = (monthsFromAppoint >= 0 && monthsFromAppoint < 12) ? (base / 12) : 0;
