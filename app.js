...
(function(){
  ...
  function setActiveShift(s){
    activeShift = s;
    shiftBtns.forEach(b=>b.classList.toggle("active", b.getAttribute("data-shift")===s));
  }
  shiftBtns.forEach(b=> b.addEventListener("click", ()=> setActiveShift(b.getAttribute("data-shift")) ));
  ...
  // ---- Multi-shift helpers ----
  function getTypes(data, k){
    const v = data[k];
    if(!v) return [];
    if(Array.isArray(v)) return v.slice();
    if(v && Array.isArray(v.types)) return v.types.slice();
    if(typeof v==="string") return v.split(/[+,|]/).map(s=>s.trim()).filter(Boolean);
    if(v && typeof v.type==="string") return [v.type];
    return [];
  }
  function setTypes(data, k, types){
    const uniq = Array.from(new Set(types)).filter(Boolean);
    if(uniq.length===0) delete data[k];
    else data[k] = uniq; // store as array
  }
  function toggleType(data, k, t){
    const cur = new Set(getTypes(data,k));
    if(cur.has(t)) cur.delete(t);
    else{
      // allow combo with NY + PR/AP; AD/RP are single-replace
      if(t==="AD" || t==="RP"){
        cur.clear(); cur.add(t);
      }else if(t==="PR" || t==="AP"){
        // remove AD/RP
        cur.delete("AD"); cur.delete("RP");
        cur.add(t);
      }else if(t==="NY"){
        cur.delete("AD"); cur.delete("RP");
        cur.add("NY");
    ...
    // if RP selected alongside others, keep RP only
    if(cur.has("RP") && cur.size>1) { cur.clear(); cur.add("RP"); }
    setTypes(data,k, Array.from(cur));
  }
  function hasService(types){
    return types.some(t => t && t!=="RP");
  }

  // ---- Pay rules (as requested) ----
  // Friday NY: ONLY 5Μ (no night pay)
  // Weekend service (Sat/Sun): +1 five-day
  // Saturday PR+NY: 1 five-day + 1 night
  // Saturday NY alone: five-day only (no night)
  function nightPaidForDate(d, types){
    const dow = d.getDay(); // 0 Sun .. 6 Sat
    if(!types.includes("NY")) return false;
    if(dow===5) return false; // Friday night => no night pay
    if(dow===6){
      // Saturday night: paid only if also worked day shift (PR or AP)
      return (types.includes("PR") || types.includes("AP"));
    }
    return true; // Sun-Thu
  }
  function fiveCountForDate(d, types){
    const dow = d.getDay();
    if(dow===5 && types.includes("NY")) return 1; // Friday NY => 5Μ
    if((dow===6 || dow===0) && hasService(types)) return 1; // weekend service => 5Μ
    return 0;
  }
  ...
  function calcCounts(y,m,data){
    ...
    Object.keys(data).forEach(k=>{
      ...
      const types = getTypes(data,k);
      if(nightPaidForDate(d, types)) nights++;
      fives += fiveCountForDate(d, types);
    });
    return {nights,fives};
  }

  function pushToPayroll(nights, fives){
    ...
    // trigger recalculation in this app (uses calc())
  ...
  function renderCal(){
    ...
    let html = `<div class="calRow">` + dows.map(d=>`<div class="calDow" style="background:#081a30;color:#fff;font-weight:900;text-align:center;padding:8px 0;border:1px solid #000">${d}</div>`).join("") + `</div>`;
    for(let r=0;r<6;r++){
      ...
      for(let c=0;c<7;c++){
        ...
        // Full-cell background for most prominent service
        let bg = "";
        if(inMonth){
          // Friday NY stays NY (blue) visually, but counts as 5Μ
          if(types.includes("NY")) bg = "#0b3d91";
          if(types.includes("AP")) bg = "#ff5a00";
          if(types.includes("PR")) bg = "#0048ff";
          if(types.includes("AD")) bg = "#7dd3fc";
          if(types.includes("RP")) bg = "#dc2626";
          // PR+NY combo gradient
          if(types.includes("PR") && types.includes("NY")) bg = "linear-gradient(135deg,#0048ff 0%,#0048ff 50%,#0b3d91 50%,#0b3d91 100%)";
        }

        const style = !inMonth ? "" : `style="background:${bg||'#0e2a50'};color:#fff;border:1px solid #163b6c;min-height:78px;padding:6px;cursor:pointer"`;
        const badges = types.map(t=>{
          const lab = t==="PR"?"ΠΡ":t==="AP"?"ΑΠ":t==="NY"?"ΝΥ":t==="AD"?"ΑΔ":t==="RP"?"Ρ":"";
          return lab ? `<div class="badge ${t}" style="margin-top:4px;background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.35);border-radius:9px;padding:2px 0;text-align:center;font-size:12px;font-weight:1000;color:#fff">${lab}</div>` : "";
        }).join("");

        const c5 = inMonth ? fiveCountForDate(d, types) : 0;
        const fiveBadge = c5 ? `<div class="badge FIVE" style="margin-top:4px;background:#ff0033;border:1px solid rgba(255,255,255,.35);border-radius:9px;padding:2px 0;text-align:center;font-size:12px;font-weight:1200;color:#fff">5Μ</div>` : "";

        const friNote = (inMonth && d.getDay()===5 && types.includes("NY")) ? `<div class="badge NOTE" style="margin-top:4px;background:rgba(0,0,0,.25);border:1px solid rgba(255,255,255,.25);border-radius:9px;padding:2px 0;text-align:center;font-size:11px;font-weight:900;color:#fff">Πα ΝΥ → 5Μ</div>` : "";

        html += `<div class="${cls}" ${style} data-key="${k}" data-inmonth="${inMonth?1:0}">
          <div class="dnum" style="font-weight:1000;color:#fff">${d.getDate()}</div>
          ${badges}
          ${fiveBadge}
          ${friNote}
        </div>`;
    ...
    // AUTO transfer to payroll every time calendar renders (after change/month nav)
    pushToPayroll(nights, fives);
  }
