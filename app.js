  ...
  function renderCal(){
    ...
    for(let r=0;r<6;r++){
      ...
      for(let c=0;c<7;c++){
        ...
        const badgesHtml = arrShifts.slice(0,2).map(t=>`<div class="badge ${t}">${labelOf(t)}</div>`).join("");
...
// ===== Calendar: delegated day click (mobile-safe) =====
(function(){
  const calGrid = document.getElementById("calGrid");
  if(!calGrid) return;

  // prevent double bind
  if(calGrid.dataset && calGrid.dataset.delegated === "1") return;
  if(calGrid.dataset) calGrid.dataset.delegated = "1";

  // Access shared variables if they exist in the main scope
  // We rely on these names from the existing calendar code:
  // - activeShift (string)
  // - schedule or data (object)
  // - save(), renderCalendar()/renderCal()
  // - normalizeShifts()
  // If the calendar uses `schedule`, map it to `data`.
  ...
  calGrid.addEventListener("click", (ev)=>{
    ...
    ev.stopPropagation();
    // CLR λειτουργεί ως "καθάρισε αυτή τη μέρα"
    if(activeShift === "CLR"){
      delete store[k];
      doSave();
      doRender();
    ...
    const cur = store[k] || "";
    const arr = (typeof normalizeShifts === "function") ? normalizeShifts(cur) : (Array.isArray(cur) ? cur.slice() : (cur ? [cur] : []));
...
// ===== Calendar click override (capture) + Clear mode + PR+NY double shift =====
(function(){
  const calGrid = document.getElementById("calGrid");
  if(!calGrid) return;

  if(calGrid.dataset && calGrid.dataset.clickOverride === "1") return;
  if(calGrid.dataset) calGrid.dataset.clickOverride = "1";
  ...
  calGrid.addEventListener("click", (ev)=>{
    ...
    // clear mode: activeShift === "CLR"
    if(typeof activeShift !== "undefined" && activeShift === "CLR"){
      delete store[k];
      doSave();
      doRender();
      return;
    ...
    if(idx >= 0){
      ...
      if(isExclusive(activeShift)){
        ...
        if(arr.length === 0){
          ...
          if(allowedDouble(existing, activeShift)){
            arr.push(activeShift);
          } else {
            // not allowed -> replace with new single
            arr.length = 0;
            arr.push(activeShift);
          }
        } else {
    ...
    if(arr.length === 0) delete store[k];
    else store[k] = arr; // store always as array for consistency
...
// ===== Bind Clear button to CLR mode (if not already) =====
(function(){
  const btn = document.getElementById("clearBtn") || document.getElementById("btnClear") || document.getElementById("clearDay") || document.querySelector('button[data-action="clear"], button#clearMonth, button#clearMonthBtn, button.btnClear');
  // We only want the top "Καθαρισμός" (day-clear mode) button if it exists with id clearBtn; otherwise do nothing.
  const dayClear = document.getElementById("clearBtn");
  if(!dayClear) return;

  dayClear.addEventListener("click", (e)=>{
    e.preventDefault();
    if(typeof activeShift !== "undefined") activeShift = "CLR";
    // visual: mark buttons
    document.querySelectorAll(".shiftBtn, .shift").forEach(b=>b.classList.remove("active"));
    dayClear.classList.add("active");
  });
})();
...
// ===== Calendar click override v4 (uses loadMonth/saveMonth/view) =====
(function(){
  if(typeof calGrid==="undefined" || !calGrid) return;
  if(calGrid.dataset && calGrid.dataset.clickOverrideV4==="1") return;
  if(calGrid.dataset) calGrid.dataset.clickOverrideV4="1";
  ...
  // Ensure "Καθαρισμός" acts as clear-mode (clear selected day)
  const clearBtn = document.getElementById("clearBtn");
  if(clearBtn && !clearBtn.dataset.boundClr){
    clearBtn.dataset.boundClr="1";
    clearBtn.addEventListener("click",(e)=>{
      e.preventDefault();
      activeShift = "CLR";
      document.querySelectorAll(".shiftBtn").forEach(b=>b.classList.remove("active"));
  ...
  calGrid.addEventListener("click",(ev)=>{
    const cell = ev.target.closest("[data-key]");
    if(!cell) return;

    // take over from any other handlers
    ev.preventDefault();
    ev.stopPropagation();
    if(ev.stopImmediatePropagation) ev.stopImmediatePropagation();
    ...
    // If click is on out-of-month day, jump view to that month first
    if(cell.getAttribute("data-inmonth")==="0"){
      const dt = parseKey(k);
      if(dt){
