  ...
  function renderCal(){
    ...
    for(let r=0;r<6;r++){
      ...
      for(let c=0;c<7;c++){
        ...
        let badgesHtml = "";
        if(two.length===1){
          badgesHtml = `<div class="badge ${two[0]}">${labelOf(two[0])}</div>`;
        } else if(two.length===2){
          badgesHtml = `<div class="badgeStack"><div class="badge ${two[0]}">${labelOf(two[0])}</div><div class="badge ${two[1]}">${labelOf(two[1])}</div></div>`;
        }
